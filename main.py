...
def encrypting(text, key): # Función para cifrar un texto
    try:
...
def decrypting(ciphertext, key): # Función para descifrar un texto
    try:
...
def main(): # Función principal
    ...
    print("Prueba de cifrado simétrico")
    print("----------------------------------------")
    ...
    if plaintext is not None: # Si el archivo no está vacío
        ...
        hash_original = get_hash(plaintext).encode()  # Convertir el hash a bytes para interpretar si se modifico el archivo
        ciphertext = encrypting(plaintext, key)  # Cifrar el texto plano
        ...
        decrypted = decrypting(current_ciphertext, key) # Descifrar el texto cifrado
